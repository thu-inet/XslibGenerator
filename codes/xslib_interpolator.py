    ...
    def _parsefolder(self):
        ...
        for file in self.folderpath.glob('*'):
            if (data:=match(self.format, file.name)) is not None:
                ...
                self.params.append(sorted(param))
    ...
    def _get_files(self, param):
        ...
        for i, param_i in enumerate(param):
            ...
            try:
                ...
                if param_i <= param[0]:
                    param_i_left = params_i[0]
                    param_i_right = params_i[1]
                else:
                    param_i_left = params_i[-2]
                    param_i_right = params_i[-1]
...
if __name__ == '__main__':
    ...
    interp = XSLIBinterpolater(args.path, format=r'trino_vercellese_ref_(\d+)K_xslib.dat')
    interp(args.params, modes=args.modes)

    # intep = XSLIBinterpolater(folderpath='/home/super/users/zhangwj/nuit/NUITLIB/numerical_benchmark')
    # intep([900, 2.72, 0, 0], modes=['linear','doppler', 'linear', 'linear'])